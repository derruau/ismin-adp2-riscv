...
if [ $# -ne 1 ]; then
    FILENAME="exo3_2ways_sans_nop.S"
else
    FILENAME="$1"
fi
...
(
    $RISCV_GCC -c -g -mabi=ilp32 -march=rv32i -O0 "$FILENAME" -o exo3.o &&
    echo "Successfully compiled '$FILENAME' to object file." &&

    $RISCV_LD --build-id=none -Bstatic -T exo3_2ways.ld -Map firmware.map exo3.o -o firmware.elf &&
